...
enum class MessageKeys(val default: String) {
    ...
    CANNOT_CAST_INSIDE_ANTIMAGICZONE("${ChatColor.LIGHT_PURPLE}Can you hear that fizzling sound? Your cast failed, there's some strange force interfering with your magic in this area."),
    CANNOT_CAST_TARGET_BLOCK_INSIDE_ANTIMAGICZONE("${ChatColor.GOLD}Woah, that was close! Seems like the targeted location is protected by some strong, mysterious force which prevents spells from interacting with it."),
    CANNOT_CAST_TARGET_ENTITY_INSIDE_ANTIMAGICZONE("${ChatColor.DARK_PURPLE}Your cast was successful, but it seems like your target is inside a area where magic cannot inflict harm upon him."),
